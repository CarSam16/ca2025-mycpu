  ...
  switch(io.opcode) {
      ...
      io.alu_funct := MuxLookup(io.funct3, ALUFunctions.zero)(
        Seq(
          ...
          InstructionsTypeI.sri   -> Mux(
        	io.funct7(5),           // funct7[5] == 1 → SRAI
        	ALUFunctions.sra,
        	ALUFunctions.srl        // funct7[5] == 0 → SRLI
        )
      )
    )
    }
    ...
    is(InstructionTypes.Op) {
      ...
      io.alu_funct := MuxLookup(io.funct3, ALUFunctions.zero)(
        ...
        io.funct7(5),           // funct7[5] == 1 → SUB
        ALUFunctions.sub,
        ALUFunctions.add        // funct7[5] == 0 → ADD
      ),
          ...
          InstructionsTypeR.sr      -> Mux(
        	io.funct7(5),           // funct7[5] == 1 → SRA
        	ALUFunctions.sra,
        	ALUFunctions.srl        // funct7[5] == 0 → SRL
      		)
        )
      )
    }
